...
def init_lattice(lattice_size):
    ...
    Returns
    -------
    numpy.ndarray, float(lattice_size, lattice_size)
      The array representing the square lattice.
    """
...
def plot_lattice(lattice, lattice_size, plot_flag):
    """
    ...
    Parameters
    ----------
    lattice : numpy.ndarray, float(lattice_size, lattice_size)
      The array representing the lattice.
    ...
    colours = np.zeros((lattice_size, lattice_size))
    ...
    if plot_flag == 1:
        ...
        for i in range(lattice_size):
            for j in range(lattice_size):
                ...
                colours[i,j] = cell_energy(lattice, lattice_size, i, j)
    ...
    quiver_options = dict(pivot='middle', headlength=0, headwidth=1, scale=(1.1 * lattice_size))
...
def save_data(lattice, lattice_size, num_steps, temperature, ratio, energy, order, runtime):
    """
    ...
    print(f"# Size of lattice:     {lattice_size:d}x{lattice_size:d}", file=file_out)
...
def cell_energy(lattice, lattice_size, x_pos, y_pos):
    """
    ...
    x_pos_right = (x_pos + 1) % lattice_size
    x_pos_left = (x_pos - 1) % lattice_size
    
    # Store the positions of the neighbouring cells in the y-direction.
    # Take into account wraparound.
    y_pos_above = (y_pos + 1) % lattice_size
    y_pos_below = (y_pos - 1) % lattice_size
...
def total_energy(lattice, lattice_size):
    """
    ...
    for i in range(lattice_size):
        for j in range(lattice_size):
            # Calculate the energy of the cell.
            energy += cell_energy(lattice, lattice_size, i, j)

    return energy


def calculate_order(lattice, lattice_size):
    """
    ...
    #
    lab = np.vstack((np.cos(lattice), np.sin(lattice), np.zeros_like(lattice))).reshape(3, lattice_size, lattice_size)
    ...
    #
    order_tensor = order_tensor / (2 * lattice_size * lattice_size)
    eigenvalues, eigenvectors = np.linalg.eig(order_tensor)

    return eigenvalues.max()


def monte_carlo_step(lattice, lattice_size, temperature):
    """
    ...
    x_positions = np.random.randint(0, high=lattice_size, size=(lattice_size, lattice_size))
    y_positions = np.random.randint(0, high=lattice_size, size=(lattice_size, lattice_size))
    
    # Generate the random angles for cell orientations.
    angles = np.random.randint(scale=angle_std, size=(lattice_size, lattice_size))
    ...
    for i in range(lattice_size):
        for j in range(lattice_size):
            ...
            energy_before = cell_energy(lattice, lattice_size, x_pos, y_pos)
            
            # Change the orientation of the cell.
            lattice[x_pos, y_pos] += angle
            energy_after = cell_energy(lattice, lattice_size, x_pos, y_pos)
    ...
    return num_accepted / (lattice_size * lattice_size)
...
def main(program_name, num_steps, lattice_size, temperature, plot_flag):
    ...
    lattice = init_lattice(lattice_size)

    # Plot the initial lattice.
    plot_lattice(lattice, lattice_size, plot_flag)
    ...
    energy[0] = total_energy(lattice, lattice_size)
    order[0] = calculate_order(lattice, lattice_size)
    ...
    for i in range(1, num_steps + 1):
        ...
        ratio[i] = monte_carlo_step(lattice, lattice_size, temperature)

        # Calculate the total energy and order parameter of the lattice.
        energy[i] = total_energy(lattice, lattice_size)
        order[i] = calculate_order(lattice, lattice_size)
    ...
    print(f"{program_name}: Size: {lattice_size:d}, Steps: {num_steps:d}, T*: {temperature:5.3f}: Order: {order[num_steps - 1]:5.3f}, Time: {runtime:8.6f} s")

    # Generate the output data file.
    save_data(lattice, lattice_size, num_steps, temperature, ratio, energy, order, runtime)
